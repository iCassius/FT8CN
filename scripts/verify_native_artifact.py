...
class ElfFacts:
    ...
    exported_dynamic_symbols: frozenset[str]
...
def _bounded_slice(data: bytes, offset: int, size: int, context: str) -> bytes:
    _validate_range(data, offset, size, context)
    return data[offset : offset + size]


def _validate_range(data: bytes, offset: int, size: int, context: str) -> None:
...
def parse_elf(data: bytes, display_path: str = "<memory>") -> ElfFacts:
    ...
    _validate_range(
        ...
        phoff,
        phentsize * phnum,
        f"{display_path}: program header table",
    )
        ...
        if int(fields[0]) == PT_LOAD:
            load_segments.append(
                LoadSegment(
                    ...
                    file_size=int(fields[ph_filesz_index]),
                    memory_size=int(fields[ph_memsz_index]),
    ...
    _validate_range(
        ...
        shoff,
        shentsize * shnum,
        f"{display_path}: section header table",
    )
    ...
    for section_index, section in enumerate(sections):
        ...
        _validate_range(
            ...
            section_offset,
            section_size,
            f"{display_path}: dynamic symbol section {section_index}",
        )
        ...
        for symbol_index in range(section_size // entry_size):
            ...
            if name_offset:
                ...
                name = _cstring(string_table, name_offset, f"{display_path}: dynamic symbol")
                info = int(symbol[sym_info_index])
                binding = info >> 4
                visibility = int(symbol[sym_other_index]) & 0x3
                section_index_value = int(symbol[sym_shndx_index])
                if section_index_value != 0 and binding in (1, 2) and visibility in (0, 3):
                    exported_symbols.add(name)
    if not saw_named_symbol:
        raise GateError(f"{display_path}: ELF has no readable dynamic symbols")
    return ElfFacts(elf_class, machine, tuple(load_segments), frozenset(exported_symbols))
    ...
    for entry in sorted(native_entries, key=lambda value: value.display_path):
        ...
        for segment_index, segment in enumerate(facts.load_segments):
            ...
            if segment.file_size > segment.memory_size:
                raise GateError(
                    f"{entry.display_path}: PT_LOAD {segment_index} file size exceeds memory size: "
                    f"0x{segment.file_size:x}>0x{segment.memory_size:x}"
                )
            _validate_range(
                entry.data,
                segment.offset,
                segment.file_size,
                f"{entry.display_path}: PT_LOAD {segment_index} file range",
            )
        actual_jni = frozenset(
            name for name in facts.exported_dynamic_symbols if name.startswith("Java_")
        )
