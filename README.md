...
- 🟡 **下一 beta**：`v0.93.005-beta.8` 已作为不可变失败 tag 保留，未创建可用 Release；当前准备独立候选 `v0.93.005-beta.9`（`93009`），实际 beta-only 签名必须由 GitHub Actions 完成，正式 Release 仍因缺受保护 formal 签名材料而阻断。
...
推送 `v*.*.*` 格式的正式 tag 后会进入 GitHub Actions formal 发布门禁；在用户确认签名迁移、提供长期 keystore 和可信证书前，formal workflow 会阻断，不创建 Release。beta tag 使用独立 beta-only workflow；`v0.93.005-beta.9` 只准备 notes 和契约，必须先让 canonical integration 与 tag peeled commit 精确相等，再通过 CI 实际签名和所有 native 门禁。
