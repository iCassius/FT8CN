...
当前事实基线为 `release@d8f8c5d`：

1. `assembleDebug` 和 `assembleRelease` 可生成 APK，但 JVM 单测为 `NO-SOURCE`，本轮没有运行 Instrumented 测试。
2. 当前 Release APK 经确认使用 Android Debug 证书，Release workflow、JDK、notes 路径和版本规则需要在稳定版前修复。
3. 网络解码断流、UDP 忙等、瀑布图并发和权限路径已有正向代码修改，但完整网络 QSO、2 小时挂机、功耗和温升仍待真机验证。
4. 后续任务和验收顺序以 [ROADMAP_TODO.md](ROADMAP_TODO.md) 为准；构建、自动化、模拟器、真机和 HIL 必须分开报告。
